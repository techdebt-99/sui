...
impl<'a> Resolver<'a> {
    ...
    pub(crate) fn instantiate_generic_field(
        ...
        let struct_type = self
            .loader
            .get_type(field_instantiation.owner)
            .ok_or_else(|| {
                PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR)
                    .with_message("Struct Definition not resolved".to_string())
            })?
...
impl Function {
    fn new(
        ...
        let (code, locals, jump_tables) = match &def.code {
            Some(code) => (
                code.code.clone(),
                Signature(
                    parameters
...
struct DataTypeInfo {
    ...
    layout: Option<MoveDataTypeLayout>,
    annotated_layout: Option<MoveDataTypeLayout>,
...
impl Loader {
    ...
    fn type_gidx_to_type_layout(
        ...
        let type_layout = match ty.data_type_info {
            ...
            DataType::Struct(ref sinfo) => {
                ...
                MoveDataTypeLayout::new(field_layouts)
            }
        };
    ...
    fn type_to_type_layout_impl(
        ...
        *count += 1;
        Ok(match ty {
            ...
            Type::DataType(gidx) => {
                MoveTypeLayout::Struct(self.type_gidx_to_type_layout(*gidx, &[], count, depth)?)
            }
            Type::DataTypeInstantiation(gidx, ty_args) => {
                MoveTypeLayout::Struct(self.type_gidx_to_type_layout(*gidx, ty_args, count, depth)?)
            }
            Type::Reference(_) | Type::MutableReference(_) | Type::TyParam(_) => {
                return Err(
                    PartialVMError::new(StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR)
    ...
    fn struct_gidx_to_fully_annotated_layout(
        ...
        let field_layouts = struct_type
            ...
            .collect::<PartialVMResult<Vec<_>>>()?;
        let struct_layout = MoveDataTypeLayout::with_types(struct_tag, field_layouts);
        let field_node_count = *count - count_before;
    ...
    fn type_to_fully_annotated_layout_impl(
        ...
        *count += 1;
        Ok(match ty {
            ...
            Type::DataType(gidx) => MoveTypeLayout::Struct(
                self.struct_gidx_to_fully_annotated_layout(*gidx, &[], count, depth)?,
            ),
            Type::DataTypeInstantiation(gidx, ty_args) => MoveTypeLayout::Struct(

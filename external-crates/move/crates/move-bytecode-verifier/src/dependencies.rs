...
fn verify_imported_structs(context: &Context) -> PartialVMResult<()> {
    let self_module = context.resolver.self_handle_idx();
    for (idx, struct_handle) in context.resolver.data_type_handles().iter().enumerate() {
        ...
        {
            Some(def_idx) => {
                let def_handle = owner_module.data_type_handle_at(*def_idx);

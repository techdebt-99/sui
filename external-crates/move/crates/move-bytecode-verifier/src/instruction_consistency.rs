...
impl<'a> InstructionConsistency<'a> {
    ...
    fn check_type_op_(
        &self,
        offset: usize,
        data_type_handle_index: DataTypeHandleIndex,
        generic: bool,
    ) -> PartialVMResult<()> {
        let data_type_handle = self.resolver.data_type_handle_at(data_type_handle_index);
        if data_type_handle.type_parameters.is_empty() == generic {
            return Err(

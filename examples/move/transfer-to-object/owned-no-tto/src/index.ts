...
import { PACKAGE_ID as PAYMENT_PACKAGE_ID } from "../gen/common/index.ts";
...
const PaymentEventType = `${PAYMENT_PACKAGE_ID}::identified_payment::PaymentEvent`;
...
let payments = await client.subscribeEvent({
    filter: { MoveEventType: PaymentEventType },
    onMessage: (event) => {
...
rl.question("Enter charge amount> ", async function(answer: string) {
    const paymentId = await makePaymentInfo(parseInt(answer));
